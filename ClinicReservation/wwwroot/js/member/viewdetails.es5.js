...
(function () {
    ...
    var btn_nextpage_click = function btn_nextpage_click() {
        ...
        if (match) {
            var page = match[1];
            if (page.length > 0) page = parseInt(page, 10) + 1;else page = 2;
            search = search.replace(/page=[^&]*/, "page=" + page);
        } else if (search.length === 0) search = "?page=2";else search = search + "&page=2";
        var url = loc + search;
        window.location.href = url;
    };
    var btn_id_filter_click = function btn_id_filter_click() {
        var btn = this.self;
        ...
        if (search.indexOf("id") > 0) search = search.replace(/id=[^&]*/, val);else if (search.length === 0) search = "?" + val;else search = search + "&" + val;

        var url = loc + search;
        window.location.href = url;
    };

    var btn_submit_message_flyoutshow = function btn_submit_message_flyoutshow(sender, args) {
        ...
        button.context.events.add("click", function (sender) {
            ...
            $.post("/member/AddDetailMessage", {
                ...
                if (ret.result) window.location.reload();else alert("false");
            });
        });
    };

    var btn_markcancel_flyoutshow = function btn_markcancel_flyoutshow(sender, args) {
        var flyoutcontext = args.flyout.context;
        var flyout = flyoutcontext.item;
        var button = $(flyout).find("div[data-control='button']")[0];
        button.context.events.clear("click");
    ...
    var btn_markcomplete_flyoutshow = function btn_markcomplete_flyoutshow(sender, args) {
        var flyoutcontext = args.flyout.context;
        var flyout = flyoutcontext.item;
        var button = $(flyout).find("div[data-control='button']")[0];
        button.context.events.clear("click");
    ...
    var accept_detail_click = function accept_detail_click(sender, args) {
    ...
    var cancelaccept_detail_click = function cancelaccept_detail_click(sender, args) {
        var btn = sender.context;
    ...
    var select_filter_changed = function select_filter_changed() {
        var loc = window.location.origin + window.location.pathname;
        ...
        if (search.indexOf("filter") > 0) search = search.replace(/filter=[^&]*/, filterval);else if (search.length === 0) search = "?" + filterval;else search = search + "&" + filterval;

        var url = loc + search;
        window.location.href = url;
    };

    var cover_click = function cover_click(sender, args) {
        var cover = this;
        ...
        var loc = window.location.origin + window.location.pathname;
        var search = window.location.search;
        if (search.indexOf("id") > 0) search = search.replace(/id=[^&]*/, "id=" + id);else if (search.length === 0) search = "?id=" + id;else search = search + "&id=" + id;
    ...
    var login_heartbeat = function login_heartbeat() {
        $.post("/member/LoginHeartbeat", {}, function (ret) {
            if (!ret.result) console.error("心跳保持失败");
        });
    };
    var load = function load() {
        ...
        if ($("#btn_nextpage")[0]) $("#btn_nextpage")[0].context.events.add("click", btn_nextpage_click);
