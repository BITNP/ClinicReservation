...
(function () {
    ...
    var save_personaldata = function save_personaldata() {
        var data = "";
    ...
    var clear_personaldata = function clear_personaldata() {
        var exp = new Date();
        exp.setTime(exp.getTime() - 1);
        document.cookie = "pdata=val;expires=" + exp.toGMTString();
    };
    var read_personaldata = function read_personaldata() {
        var arr,
            reg = new RegExp("(^| )" + "pdata" + "=([^;]*)(;|$)");
        var chs = $("#checkbox_savepdata");
        if (arr = document.cookie.match(reg)) {
            var data = arr[2].split("|xx|");
            if (chs.length > 0) $("#checkbox_savepdata")[0].context.set_checked(true);
            ...
            if (chs.length > 0) chs[0].context.set_checked(false);
        }
    };
    ...
    var check_all = function check_all() {
        ...
        if (enablesubmit) $("#btn_submit")[0].context.set_enabled(true);else $("#btn_submit")[0].context.set_enabled(false);
    };

    var copy_values = function copy_values() {
    ...
    var focus_name = function focus_name() {
        $("#input_name_hint").addClass("show");
    };
    var blur_name = function blur_name() {
        $("#input_name_hint").removeClass("show");
    };
    var focus_phonenumber = function focus_phonenumber() {
        $("#input_phone_hint").addClass("show");
    };
    var blur_phonenumber = function blur_phonenumber() {
        $("#input_phone_hint").removeClass("show");
        check_phonenumber();
    };
    var check_phonenumber = function check_phonenumber() {
        var reg = /^\d+$/gi;
    ...
    var focus_email = function focus_email() {
    ...
    var blur_email = function blur_email() {
    ...
    var focus_qq = function focus_qq() {
        $("#input_qq_hint").addClass("show");
    };
    var blur_qq = function blur_qq() {
    ...
    var submit_click = function submit_click(sender) {
        sender.context.set_enabled(false);
        $("#load_ring")[0].style.opacity = 1;

        var save = false;
        if ($("#checkbox_savepdata")[0]) save = $("#checkbox_savepdata")[0].context.is_checked();
        if (save) save_personaldata();else clear_personaldata();
